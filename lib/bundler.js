if (process.versions.node) {
  ...
  function addHmrToBundles(socketHostname, port, blocksDir) {
    const { bundles, importStatement } = blockTypes.js;
    const hmrPath = `${require.resolve.paths('./hmr-client.js')[0]}/hmr-client.js`;
    const hmrContent = fs.readFileSync(hmrPath, 'utf8').replace('SOCKET_HOSTNAME', socketHostname).replace('PORT', port);
  ...
  function start(opts) {
    ...
    const { blocksDir, root, watch, socketHostname, port } = opts;
    ...
    if (watch) {
      ...
      addHmrToBundles(socketHostname, port, blocksDir);
      hmr.start(opts);
