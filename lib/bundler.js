if (process.versions.node) {
  ...
  function makeMap(opts) {
    const { templates, handlePaths, assets, origin } = opts;
  ...
  function makeAssetsMap(outputs, opts) {
    Object.entries(outputs).forEach(([path, value]) => {
      ...
      if (['.css', '.js', '.svg'].includes(extname(key))) {
        const assetPath = resolve(__base, '../public', hashedPath.replace(/^\//, ''));
        opts.assets[key].content = fs.readFileSync(assetPath, 'utf8');
      }

      opts.assets[key].path = key.match(/\.svg$/)
        ? hashedPath
        : `${opts.origin}${hashedPath}`;
    });
  }
