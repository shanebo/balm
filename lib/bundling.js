...
const removeExtension = (path) => path.replace(/\.balm$/, '');
...
function addHmrToBundles(blocksDir) {
  const { bundles, importStatement } = blockTypes['js'];
  const hmrDir = require.resolve.paths('./hmr.js')[0];
  const hmrPath = `${hmrDir}/hmr.js`;

  Object.keys(bundles).forEach((bundle) => {
    bundles[bundle].push(importStatement(relative(blocksDir, hmrPath)));
  });
}
